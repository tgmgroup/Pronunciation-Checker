document.addEventListener("DOMContentLoaded", () => {
    ...
    const sampleTexts = [
        ...
        "This website helps you practice your English pronunciation.",
    ...
    try {
        ...
        if (textFromUrl !== null && textFromUrl.trim() !== "") {
            ...
            statusDiv.textContent = "ステータス：準備OK。";
    ...
    function loadRandomSample(updateStatus = true) {
        ...
        if (sampleTexts && sampleTexts.length > 0) {
            ...
            statusDiv.textContent = "ステータス：例が見つかりません。";
        }
    }
    ...
    function populateVoiceList() {
        ...
        if (uniqueVoices.length === 0) {
            statusDiv.textContent = "ステータス：声が見つかりません。";
            return;
        }
    ...
    speakButton.addEventListener("click", () => {
        ...
        const selectedVoice = synthVoices.find(
        ...
        utterance.onstart = () => {
            ...
            statusDiv.textContent = "ステータス：発話中...";
        };
        utterance.onend = () => {
        ...
        utterance.onerror = (event) => {
            statusDiv.textContent = `ステータス：TTS問題がありました： ${event.error}`;
    ...
    if (!SpeechRecognition) {
        ...
        recognition.onerror = (event) => {
            statusDiv.textContent = `ステータス：問題がありました： ${event.error}`;
    ...
    recordButton.addEventListener("click", () => {
        if (!isRecording && recognition) {
            ...
            try {
                ...
                statusDiv.textContent = `ステータス：録音が出来ませんでした： ${error.message}`;
    ...
    function compareText() {
        ...
        if (!originalText || !recognizedText) {
            feedbackDiv.textContent = "問題がありました：テキストが見つかりませんでした。";
        ...
        feedbackDiv.textContent = `正確度： ${accuracy}%`; // Added note
